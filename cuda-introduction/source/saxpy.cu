...
int main(int argc, char *argv[])
{
    ...
    // Device arrays
    float *d_x, *d_y, *d_z;
    // float *d_null = nullptr; // used for error checking
    ...
    // CUDA(cudaMemcpy(d_null, y, size * sizeof(float), cudaMemcpyHostToDevice));
    // No need to copy z, since we will write to it.
